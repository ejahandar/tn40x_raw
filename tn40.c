...
void raw_access_init(void)
{
	...
	int tmp_major = register_chrdev(0, "ethraw", &chrdev_fops);
	...
	DBG("%s: %s got major %i for %s\n", BDX_DRV_NAME, ndev->name, , raw_dev_major, RAW_ACCESS_DEV_NAME);
}
...
{	
	...
	priv->chrdev_rx_buffer = kmalloc(RAW_ACCESS_BUFFER_SIZE, GFP_KERNEL);
...
int chrdev_open(struct inode *inode, struct file *file)
{
	...
	struct bdx_priv *priv = adapter_priv[minor];
...
ssize_t chrdev_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	struct bulk_trx_d * trx_desc = priv->chrdev_tx_buffer;
	...
	char * frame_ptr = trx_desc->frame_payload;
	
	DBG("%s: %s Sending %i frames\n", BDX_DRV_NAME, ndev->name, trx_desc->no_frames);
	
	for(i=0;i<trx_desc->no_frames;i++){
		frame_size = trx_desc->frame_size[i];
		frame_ptr = &trx_desc->frame_payload[i*RAW_ACCESS_MAX_FRAME_SIZE];
		if(frame_size > 0 && frame_size < RAW_ACCESS_MAX_FRAME_SIZE && (frame_bytes_read + frame_size) < len){
			...
			DBG("%s: %s Sending FRAME %i : 0x%x [%i]\n", BDX_DRV_NAME, ndev->name, i, frame_ptr, frame_size);
