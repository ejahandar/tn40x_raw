...
void setMDIOSpeed(void __iomem *regs, int speed)
{
	int mdio_cfg;
...
//	else
//	{
//		DBG("bdx_mdio_look_for_phy() no PHY on port 0x%x\n", port);
		for (i = 0; i < 32; i++)
		{
			msleep(10);
...
#ifdef PHY_AQ2104
		case 0x03A1B462:   //AQ2104 B0
		case 0x03A1B463:   //AQ2104 B1
		case 0x03A1B4A3:   //AQR105 B1

			s="AQ2104 10Gbps 10GBase-T";
			setMDIOSpeed(regs,6);
			*phy_t = AQ2104_register(priv);
			break;
#endif
...
void bdx_speed_changed(struct bdx_priv *priv, u32 speed)
{
	...
	switch(speed)
	{
		...
		case SPEED_5000:
			if(speed == SPEED_5000) priv->link_speed=SPEED_5000;
		case SPEED_2500:
			if(speed == SPEED_2500) priv->link_speed=SPEED_2500;
...
static  void bdx_setAffinity(u32 irq)
{
	...
	DBG(BDX_DRV_NAME ": bdx_setAffinity  nr_cpu_ids 0x%x\n", nr_cpu_ids);
	DBG(BDX_DRV_NAME ": bdx_setAffinity  cpu_online_mask 0x%x\n", *(u32*)cpu_online_mask);
	if (irq_set_affinity_hint(irq, cpu_online_mask))
	{
		ERR(BDX_DRV_NAME ": bdx_setAffinity() irq_set_affinity_hint failed!\n");
	}
...
static void bdx_hw_stop(struct bdx_priv *priv)
{
	...
	if (priv->state != BDX_STATE_HW_STOPPED)
	{
...
static int bdx_tx_transmit(struct sk_buff *skb, struct net_device *ndev)
{
	...
	*/
	...
	if (priv->tx_level > priv->tx_update_mark)
	{
		...
		WRITE_REG(priv, f->m.reg_WPTR, f->m.wptr & TXF_WPTR_WR_PTR);
		DBG("WRITE_REG 0x%04x f->m.reg_WPTR 0x%x\n", f->m.reg_WPTR, f->m.wptr & TXF_WPTR_WR_PTR);
		DBG("READ_REG  0x%04x f->m.reg_RPTR=0x%x\n", f->m.reg_WPTR, READ_REG(priv, f->m.reg_RPTR));
	}
	else
	{
		if (priv->tx_noupd++ > BDX_NO_UPD_PACKETS)
		{
			priv->tx_noupd = 0;
			WRITE_REG(priv, f->m.reg_WPTR, f->m.wptr & TXF_WPTR_WR_PTR);
			DBG("WRITE_REG(1) 0x%04x f->m.reg_WPTR 0x%x\n", f->m.reg_WPTR, f->m.wptr & TXF_WPTR_WR_PTR);
			DBG("READ_REG  0x%04x f->m.reg_RPTR=0x%x\n", f->m.reg_WPTR, READ_REG(priv, f->m.reg_RPTR));
...
//		bdx_ethtool_ops(ndev);  /* Ethtool interface */
		...
		minor = chrdev_init(priv, (char *) &ndev->name);
		
		if(minor == -1){
			ERR(BDX_DRV_NAME ": could not register character device %s\n", (char *) &ndev->name);
		}else{
			printk(KERN_INFO "%s: registered raw access for %s via /dev/%s (%i,%i) \n", BDX_DRV_NAME, (char *) &ndev->name, (char *) &ndev->name, priv->chrdev_major, priv->chrdev_minor);
		}
...
static void __exit bdx_remove(struct pci_dev *pdev)
{
	...
	for (port = 0; port < nic->port_num; port++)
	{
		...
		bdx_rx_free(priv);
		bdx_tx_free(priv);
		chrdev_cleanup(priv, (char *) &ndev->name);
...
void raw_access_init(void)
{
	...
	for(i=0;i<MAX_NO_ETH_ADAPTERS;i++){
		adapter_priv[i] = NULL;
	}
	
	tmp_major = register_chrdev(0, "ethraw", &chrdev_fops);
	
	if(tmp_major < 0){
		ERR(BDX_DRV_NAME " : registering major number for char device failed\n");
		return;
	}
	
	// Register the device class
	chrdev_class = class_create(THIS_MODULE, RAW_ACCESS_DEV_NAME);
	
	// Check for error and clean up if there is
	if(IS_ERR(chrdev_class)){

		unregister_chrdev(tmp_major, RAW_ACCESS_DEV_NAME);
		chrdev_class = NULL;
		
		ERR(BDX_DRV_NAME " : Failed to register device class\n");

		// Correct way to return an error on a pointer
		return;
	}
	
	raw_dev_major = tmp_major;
	DBG("%s: got major %i for %s\n", BDX_DRV_NAME, raw_dev_major, RAW_ACCESS_DEV_NAME);
}

void raw_access_deinit(void)
{
	if(chrdev_class != NULL){		
		...
		unregister_chrdev(raw_dev_major, RAW_ACCESS_DEV_NAME);
	}
}
...
{	
	...
	priv->chrdev_device = device_create(chrdev_class, NULL, MKDEV(raw_dev_major, minor), NULL, name);
	
	// Clean up if there is an error
	if (IS_ERR(priv->chrdev_device)){
		ERR(BDX_DRV_NAME " : Failed to create the character device class\n");
		return -1;
	}

	priv->chrdev_rx_buffer_A = kmalloc(RAW_ACCESS_BUFFER_SIZE, GFP_KERNEL);
	priv->chrdev_rx_buffer_B = kmalloc(RAW_ACCESS_BUFFER_SIZE, GFP_KERNEL);
	priv->chrdev_tx_buffer = kmalloc(RAW_ACCESS_BUFFER_SIZE, GFP_KERNEL);
	
	if(priv->chrdev_rx_buffer_A != NULL && priv->chrdev_rx_buffer_B != NULL && priv->chrdev_tx_buffer != NULL){
		priv->chrdev_initialized = true;
	}else{
		return -1;
	}
...
void chrdev_cleanup(struct bdx_priv *priv, char * name)
{
	if(priv->chrdev_major > 0 && priv->chrdev_minor >= 0){
		// remove the device
		device_destroy(chrdev_class, MKDEV(priv->chrdev_major, priv->chrdev_minor));
		if(priv->chrdev_rx_buffer_A != NULL){
			kfree(priv->chrdev_rx_buffer_A);
		}
		if(priv->chrdev_rx_buffer_B != NULL){
			kfree(priv->chrdev_rx_buffer_B);
		}
		if(priv->chrdev_tx_buffer != NULL){
			kfree(priv->chrdev_tx_buffer);
		}
		adapter_priv[priv->chrdev_minor] = NULL;
		
		priv->chrdev_initialized = false;
		priv->chrdev_device = NULL;
		priv->chrdev_device_open = false;
		priv->chrdev_major = -1;
		priv->chrdev_minor = -1;
	}
...
ssize_t chrdev_write(struct file *filp, const char *buff, size_t len, loff_t * off)
{
	...
	trx_desc = (struct bulk_trx_d *) priv->chrdev_tx_buffer;
	
	if(trx_desc->no_frames <= 0){
		return -EINVAL;
	}
	...
	for(i=0;i<trx_desc->no_frames;i++){
		...
		if(frame_size > 0 && frame_size < RAW_ACCESS_MAX_FRAME_SIZE && (frame_bytes_read + frame_size) < len){
			...
			DBG("%s: %s Sending FRAME %li : 0x%p [%i]\n", BDX_DRV_NAME, priv->ndev->name, i, frame_ptr, frame_size);
			
			bdx_tx_transmit_raw(frame_ptr, frame_size, priv->ndev);
			frame_bytes_read += frame_size;
		}
	}
...
int bdx_tx_transmit_raw(char * raw_data, int length, struct net_device *ndev)
{
	...
	txdd->length   = CPU_CHIP_SWAP16(length);
	txdd->mss	  = CPU_CHIP_SWAP16(txd_mss);
	txdd->txd_val1 = CPU_CHIP_SWAP32(TXD_W1_VAL(txd_sizes[nr_frags].qwords,
									 txd_checksum, txd_vtag,
									 txd_lgsnd, txd_vlan_id));
	...
	bdx_tx_map_raw(priv, raw_data, length, txdd);
	...
	*/
	...
	priv->net_stats.tx_packets++;
	priv->net_stats.tx_bytes += length;	
...
static inline void bdx_tx_map_raw(struct bdx_priv *priv, char * raw_data, int length, struct txd_desc *txdd){
	...
	db->wptr->len	  = length;
	db->wptr->addr.dma = pci_map_single(priv->pdev, raw_data, db->wptr->len, PCI_DMA_TODEVICE);
	...
	db->wptr->len	  = -txd_sizes[nr_frags].bytes;
	db->wptr->addr.skb = NULL; // cleanup routine knows to don't free NULL skb
	bdx_tx_db_inc_wptr(db);
}
